...
class Preprocessor:
    ...
    def __init__(self, path_in: str, path_out: str, loader: str, cleaning_type: str, remove_brackets: bool, remove_emojis: bool) -> None:
        self.path_in = path_in
        self.path_out = path_out
        self.loader = LOADERS[loader]
        self.writer = WRITERS['JSONLWriter']
        self.cleaner = Cleaner(cleaning_type=cleaning_type, remove_brackets=remove_brackets, remove_emojis=remove_emojis)
...
class EDOSTaskAPreprocessor(Preprocessor):
    
    def process(self) -> None:
        entries = self.loader.load(self.path_in)
        std_entries = []
        for entry in entries:
            std_entry = self.create_standard_entry(
                    id_=entry['rewire_id'], 
                    text=self.cleaner.clean(entry['text']), 
                    label_type='task_A', 
                    label_value=LABEL_STR_TO_LABEL_NUM[entry['label_sexist']], 
                    source=self.corpus_name,
                )
            std_entries.append(std_entry)
        self.writer.write(entries, self.path_out)


class EDOSTaskBPreprocessor(Preprocessor):
...
class EDOSTaskCPreprocessor(Preprocessor):
...
#     @staticmethod
#     def create_standard_entry(id_: Union[str, int], text: str, label_type: str, label_value: int, source: str
#                               ) -> Dict[str, Union[int, str]]:
#         """Create a training/dev entry (dict).

#         The method standardizes what an item-dict contains.
#         """
#         return {'id': id_, 'text': text, 'label_type': label_type, 'label_value': label_value, 'source': source}
